...
(cd /usr/local/var ; git clone https://github.com/patchvonbraun/gr-ra_blocks.git)
(cd gr-ra_blocks
cd trunk
cmake .
make
...
(cd /usr/local/var ; git clone https://github.com/patchvonbraun/simple_ra)
(cd /usr/local/var/simple_ra ; sudo make install)
